    ...
    const paginationItems = () => {
        if (isManyPages) {
            return (
                <>
                    {
                        currentPage <= 5 ? pagesArray.slice(0, currentPage-1).map(page => (
                            <PaginationItem key={page} page={page} setCurrentPage={setCurrentPage} />
                        )) : (
                            <>
                                <PaginationItem page={1} setCurrentPage={setCurrentPage} />
                                ... 
                                {pagesArray.slice(currentPage - 4, currentPage-1).map(page => (
                                    <PaginationItem key={page} page={page} setCurrentPage={setCurrentPage} />
                                ))}
                            </>)
                    }
                        <PaginationItem page={currentPage} className={'current'} setCurrentPage={setCurrentPage} />
                    {
                        currentPage >= pages - 4 ? 
                            pagesArray.slice(currentPage, pages).map(page => (
                                <PaginationItem key={page} page={page} setCurrentPage={setCurrentPage} />
                            ))
