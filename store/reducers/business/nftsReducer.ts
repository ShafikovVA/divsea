...
const fetchNfts = () => {
    ...
    return createAsyncThunk('nfts/fetchCards', async ({page, filters}: {page?: number, filters?: INftsFilters}) => {
    if (page) {
        pageValue = page;
    }
    if (filters) {
    ...
    const queryString = new URLSearchParams({...filtersValue, _page: pageValue?.toString() || ''}).toString();
    try {
        const response = await fetch(`http://localhost:3001/cards${queryString ? '?'+queryString : ''}`);
